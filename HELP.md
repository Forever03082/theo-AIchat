...
1. 开放https://dashboard.cpolar.com：

寻找左侧验证或Authtoken复制令牌。

2. 在CMD里执行命令：
```Bash
   cpolar authtoken 你的token
```
3. 现在确保Spring Boot项目正在运行，然后执行命令：
```Bash
   cpolar http 8080
```
就会生成一个公网地址。
...
2. 切换到你的项目数据库
   ...
   登录成功后，进入项目数据库 ai_knowledge_base：
...
3. 顺次执行基础 SQL 擦除数据
   ...
   假设你要删除的账号的用户名是 test123@gmail.com，请在命令行中依次复制并执行这三行 SQL：
